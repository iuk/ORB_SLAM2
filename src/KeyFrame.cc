 ...
 * 2. 并且该权重必须大于一个阈值，如果没有超过该阈值的权重，
 *    那么就只保留权重最大的边（与其它关键帧的共视程度比较高）
 * 3. 对这些连接按照权重从大到小进行排序，以方便将来的处理
 *    更新完covisibility图之后，如果没有初始化过，则初始化为连接权重最大的边
 *   （与其它关键帧共视程度最高的那个关键帧），类似于最大生成树
 */
void KeyFrame::UpdateConnections() {
  ...
  int th           = 15;    // 权重15,意味着共视点数>15
  int nmax         = 0;     // 记录最大权重
  KeyFrame *pKFmax = NULL;  //记录最共视的keyframe
  ...
  //===============3==================================
  {
    ...
    // 更新图的连接(权重)
    // 更新该KeyFrame的mConnectedKeyFrameWeights，更新当前帧与其它关键帧的连接权重
