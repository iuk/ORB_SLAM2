...
Frame::Frame(const cv::Mat &imGray, const double &timeStamp, ORBextractor *extractor,
  ...
  // This is done only for the first Frame (or after a change in the calibration)
  // 第一帧的时候计算这些信息，这些信息与相机参数有关，只计算一次就可以
  if (mbInitialComputations) {
    // 三、计算四边去畸变后的坐标
    // 存在 float Frame::mnMinX, Frame::mnMinY, Frame::mnMaxX, Frame::mnMaxY;
    ComputeImageBounds(imGray);
...
void Frame::ExtractORB(int flag, const cv::Mat &im) {
  // 这里调用的是 ORBextractor::operator() 函数
  // 输入 图片 蒙版
  // 输出 keypoint 描述子
  if (flag == 0)
...
// 从 4x4T 中 提取 3x3R 和 3x1t
void Frame::UpdatePoseMatrices() {
  mRcw = mTcw.rowRange(0, 3).colRange(0, 3);  // 3x3旋转矩阵，世界系下的相机姿态
  mRwc = mRcw.t();
  mtcw = mTcw.rowRange(0, 3).col(3);  // 3x1平移向量，世界系下的相机的位置
  mOw = -mRcw.t() * mtcw;
}
