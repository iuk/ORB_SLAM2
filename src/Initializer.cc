...
bool Initializer::Initialize(
    const Frame &CurrentFrame,      // 输入 初始化的第二帧
    const vector<int> &vMatches12,  // 输入 1-2帧匹配keypoint的id, 位置是 第1帧的，id 是第二帧的
    cv::Mat &R21,                   // 输出 两帧间相机旋转
    cv::Mat &t21,                   // 输出 两帧间相机平移
    vector<cv::Point3f> &vP3D,      // 输出 特征点的 三维点坐标
    vector<bool> &vbTriangulated)   // 输出 点对是否可以三角化
{
